...
class SimpleImage {
  ...
  public function crop($x1, $y1, $x2, $y2) {
    ...
    $this->image = imagecrop($this->image, ['x' => min($x1, $x2), 'y' => min($y1, $y2),
  ...
  function duotone($lightColor, $darkColor) {
    ...
    for ($x = 0; $x < $this->getWidth(); $x++) {
      for ($y = 0; $y < $this->getHeight(); $y++) {
        ...
        $this->dot($x, $y, ['red' => $pixels['red'][$gray], 'green' => $pixels['green'][$gray],
  ...
  public function text($text, $options, &$boundary = null) {
    ...
    $options = array_merge(['fontFile' => null, 'size' => 12, 'color' => 'black',
      'anchor' => 'center', 'xOffset' => 0, 'yOffset' => 0, 'shadow' => null], $options);
    ...
    $x = (int)round($x);
    $y = (int)round($y);
    $boundary = ['x1' => $x, 'y1' => $y - $boxHeight, 'x2' => $x + $boxWidth, 'y2' =>
  ...
  public function blur($type = 'selective', $passes = 1) {
    $filter = $type === 'gaussian' ? IMG_FILTER_GAUSSIAN_BLUR :
  ...
  public function colorize($color) {
    ...
    imagefilter($this->image, IMG_FILTER_COLORIZE, $color['red'], $color['green'], $color['blue'],
      127 - ($color['alpha'] * 127));
    return $this;
  }
  public function contrast($percentage) {
    imagefilter($this->image, IMG_FILTER_CONTRAST, self::keepWithin($percentage, -
      100, 100));
    return $this;
  }
  ...
  public function opacity($opacity) {
    ...
    self::imageCopyMergeAlpha($newImage->image, $this->image, $x, $y, 0, 0, $this->
      getWidth(), $this->getHeight(), self::keepWithin($opacity, 0, 1) * 100);
    return $this;
  }
  ...
  public static function adjustColor($color, $red, $green, $blue, $alpha) {
    $color = self::normalizeColor($color);
    return self::normalizeColor(['red' => $color['red'] + $red, 'green' => $color['green'] +
      $green, 'blue' => $color['blue'] + $blue, 'alpha' => $color['alpha'] + $alpha]);
  }
  ...
  public function extractColors($count = 5, $backgroundColor = null) {
    if (!class_exists('\League\ColorExtractor\ColorExtractor')) {
      throw new \Exception('Required library \League\ColorExtractor is missing.', self::
        ERR_LIB_NOT_LOADED);
    }
    if ($backgroundColor) {
      $backgroundColor = self::normalizeColor($backgroundColor);
      $backgroundColor = \League\ColorExtractor\Color::fromRgbToInt(['r' => $backgroundColor['red'],
        'g' => $backgroundColor['green'], 'b' => $backgroundColor['blue']]);
    }
    ...
    foreach ($colors as $key => $value) {
      $colors[$key] = self::normalizeColor(\League\ColorExtractor\Color::fromIntToHex
        ($value));
    }
    return $colors;
  }
  ...
  public static function normalizeColor($color) {
    $cssColors = ['aliceblue' => '#f0f8ff', 'antiquewhite' => '#faebd7', 'aqua' =>
      ...
      '#483d8b', 'darkslategray' => '#2f4f4f', 'darkslategrey' => '#2f4f4f',
      'darkturquoise' => '#00ced1', 'darkviolet' => '#9400d3', 'deeppink' => '#ff1493',
      'deepskyblue' => '#00bfff', 'dimgray' => '#696969', 'dimgrey' => '#696969',
      'dodgerblue' => '#1e90ff', 'firebrick' => '#b22222', 'floralwhite' => '#fffaf0',
      'forestgreen' => '#228b22', 'fuchsia' => '#ff00ff', 'gainsboro' => '#dcdcdc',
      'ghostwhite' => '#f8f8ff', 'gold' => '#ffd700', 'goldenrod' => '#daa520', 'gray' =>
      '#808080', 'grey' => '#808080', 'green' => '#008000', 'greenyellow' => '#adff2f',
      ...
      '#d3d3d3', 'lightgrey' => '#d3d3d3', 'lightgreen' => '#90ee90', 'lightpink' =>
      ...
      '#0000cd', 'mediumorchid' => '#ba55d3', 'mediumpurple' => '#9370db',
      ...
      '#d2b48c', 'teal' => '#008080', 'thistle' => '#d8bfd8', 'tomato' => '#ff6347',
      'turquoise' => '#40e0d0', 'violet' => '#ee82ee', 'wheat' => '#f5deb3', 'white' =>
      '#ffffff', 'whitesmoke' => '#f5f5f5', 'yellow' => '#ffff00', 'yellowgreen' =>
      '#9acd32'];
    ...
    if (is_string($color)) {
      ...
      $color = ['red' => hexdec($red), 'green' => hexdec($green), 'blue' => hexdec($blue),
        'alpha' => $alpha];
    }
    if (is_array($color)) {
      ...
      return ['red' => (int)self::keepWithin((int)$color['red'], 0, 255), 'green' => (int)
        self::keepWithin((int)$color['green'], 0, 255), 'blue' => (int)self::keepWithin((int)
        $color['blue'], 0, 255), 'alpha' => self::keepWithin($color['alpha'], 0, 1)];
    }
    throw new \Exception("Invalid color value: $color", self::ERR_INVALID_COLOR);
  }
}
